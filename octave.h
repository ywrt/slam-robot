...
class Octave {
 public:
  ...
  // Search around a radius of the current position in this octave for the
  // best match of the patch at the current positions. If a good match is
  // found, then this is probably a bad corner (it looks very similar when
  // moved a short distance).
  int ScorePosition(const FPos &fp, int radius) const;
